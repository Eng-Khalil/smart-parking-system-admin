...
export default async function page() {
  ...
  const lotsData:any[] = (await getAllParkingLots()) || [];
  ...
  const parkingLots=lotsData.map((item:any,i:any)=>{
    return(
      {
        label:item.name,
       value:item.id,
  ...
  return (
    <div className="p-8">
      <SpotForm parkingLots={parkingLots} regions={regions} cities={cities} areas={areas} streets={streets}/>
    </div>
  );
}
